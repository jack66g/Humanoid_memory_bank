...
class CognitiveEngine:
    ...
    def process_pending_queue(self):
        ...
        while self.pending_queue:
            ...
            # ==========================================
            # 【改动 3】：融合/覆盖机制，插入逻辑裁判员
            # ==========================================
            if M > self.config.m_consolidation_threshold and old_id is not None:
                
                # 调用裁判员判断是否冲突/改口
                is_conflict = self.evaluate_csn_with_llm.__self__.detect_conflict(old_text, user_text)
                
                if is_conflict:
                    # 💥 分支 A：发生歧义！用户改口了，执行【覆盖软删除】
                    print(f"  ⚡ [观念修正] 发现歧义！用户改口了。\n  旧：{old_text[:20]}...\n  新：{user_text[:20]}...")
                    
                    old_meta["initial_v"] = 0.1  # 强行衰减价值
                    old_meta["is_deprecated"] = True # 标记为废弃
                    self.long_term_db.update(ids=[old_id], metadatas=[old_meta])
                    
                    # 核心：这里绝对不能写 continue！
                    # 我们要把 user_text 放行，让它顺利往下走，作为一条全新的高分记忆入库！
                else:
