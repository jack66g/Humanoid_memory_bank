...
# 导入你的认知皮层核心
from cogni_memory3 import CogniCore
from cogni_memory3.config import CogniConfig
...
def chat_with_cognicore(user_message, history, top_k_val):
    ...
    try:
        ...
        # 2. 组装多层上下文 (三层汉堡包注入)
        # ==========================================
        # 第一层：System 潜意识层（搭载长期记忆）
        system_instruction = (
            "你是一个有自我意志的数字生命实体。请结合你的【长期记忆】和当前的【短期对话上下文】来自然地回答用户。\n"
            "注意：如果当前的短期对话与长期记忆有逻辑冲突，请以当前的短期对话为准。\n\n"
            f"【触发的长期记忆】：\n{memory_context}"
        )
        messages = [{"role": "system", "content": system_instruction}]
        
        # 第二层：History 聊天记录层（传统上下文记忆）
        for msg in history:
            messages.append({"role": msg["role"], "content": msg["content"]})
            
        # 第三层：Current 最新输入层（当前触点）
        ...
        payload = {
            "model": MODEL_NAME,
            "messages": messages, # <--- 【核心改动】：把完整的三层结构发给大模型
            "temperature": 0.7,
            "max_tokens": 4096
        }
        ...
        # ==========================================
        # 【核心新增】：组装近期对话上下文 (Working Memory)
        # ==========================================
        recent_context = ""
        # 提取最近的 3 轮对话作为裁判员判断冲突的上下文支撑
        ...
        # 4. 感知与物理代谢！
        # 注意：这里传给 user_text 的不再是单单一句话，而是包含前文的 recent_context
        engine.perceive(user_text=recent_context, ai_text=ai_reply)
        engine.sleep()
